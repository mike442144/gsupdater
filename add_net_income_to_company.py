...
INDIRECT formulas that reference those rows — they must be fixed in a separate pass.

Usage:
    python add_net_income_to_company.py --spreadsheet-id <id>            # one spreadsheet
    python add_net_income_to_company.py --spreadsheet-id <id> --dry-run  # preview only
    python add_net_income_to_company.py                                  # all industries
    python add_net_income_to_company.py --dry-run
"""
...
def scan_tab(service, spreadsheet_id, sheet_name):
    ...
    for i, row in enumerate(rows):
        ...
        if current_section == 'key stats':
            low = item_val.lower()
            if low == NET_INCOME_TO_COMPANY_LABEL.lower():
                net_income_to_company_row = i
            elif low == 'net income':
                net_income_row = i
...
def process_tab(service, spreadsheet_id, sheet_name, sheet_id, dry_run):
    ...
    requests = []
    if net_income_to_company_row is not None:
        # Re-runnable: rewrite formulas in the existing row, no insert/shift.
        target_row = net_income_to_company_row
        shifted = item_to_row
        mode = 'rewrite'
    else:
    ...
    requests.append({
        'updateCells': {
            ...
            'rows': [{'values': [{'userEnteredValue': {'stringValue': NET_INCOME_TO_COMPANY_LABEL}}]}],
    ...
    for dc in data_cols:
        cl = col_to_letter(dc)
        formula = resolve_formula(NET_INCOME_TO_COMPANY_TEMPLATE, cl, shifted, sheet_name)
        ...
        requests.append({
            'updateCells': {
                ...
                'rows': [{'values': [{
                    ...
                    'userEnteredFormat': {'numberFormat': dict(NUMBER_FORMAT)},
                }]}],
    ...
    if dry_run:
        ...
        print(f"  {sheet_name}: row {target_row + 1} {lc}{target_row + 1}=\"{NET_INCOME_TO_COMPANY_LABEL}\"  "
    ...
    verb = ('rewrote formula in existing row' if mode == 'rewrite'
            else f'inserted Net Income to Company row at {target_row + 1},')
...
def main():
    parser = argparse.ArgumentParser(description='Add Net Income to Company row to Key Stats')
